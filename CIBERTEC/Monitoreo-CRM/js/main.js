...
const targetColumns = {
    ...
    estado: "Estado",
    fecha: "Fecha",
    hora: "Hora", // Cibertec no trae columna de hora separada; el motor la extrae del propio valor de Fecha
    periodo: "Periodo",
    ...
    idConversacion: "ID Conversación" // Cibertec usa Genesys en vez de Chattigo
};

const requiredColumns = ["Código de Persona", "Fecha"]; // los únicos que el sistema realmente necesita
...
const colMappingKeywords = {
    ...
    // ID Conversación (Cibertec usa Genesys en vez de Chattigo)
    "genesys conversation id": "ID Conversación",
    "id conversacion": "ID Conversación",
    "id conversación": "ID Conversación",

    // Fuente Origen: Cibertec exporta con paréntesis distintos a UPC
    "fuente de origen (contacto) (contacto)": "Fuente Origen"
};
...
function switchTab(showId, hideId, activeBtn) {
    ...
    btns.forEach(btn => btn.classList.remove("active", "text-cib-blueDark"));
    activeBtn.classList.add("active", "text-cib-blueDark");
}
...
function normalizeChannel(rawChannel) {
    ...
    let parsed = "";
    // Fix: use full string before "/" (not just first word) to avoid truncating
    // values like "Otras fuentes/Otras fuentes" → would become "Otras" incorrectly
    if (rawText.includes("/")) {
        parsed = rawText.split("/")[0].trim() || "Otros";
    } else {
        parsed = rawText.trim() || "Otros";
    }
    ...
    return parsed.toLowerCase().replace(/\b\w/g, c => c.toUpperCase());
}

// TODO: confirmar con Cibertec si aplica — Horario comercial: L-V 9:00-21:00, Sáb 9:00-19:00, Dom no laboral
function isWithinBusinessHours(dateTime) {
    ...
    if (day >= 1 && day <= 5) return minutesOfDay >= 540 && minutesOfDay < 1260; // L-V 09:00-21:00
    if (day === 6) return minutesOfDay >= 540 && minutesOfDay < 1140; // Sáb 09:00-19:00
    return false;
}
...
function parseChannelAndSegment(rawValue, existingSegment = "") {
    ...
    if (!rawText) {
        return {
            canal: "Otros",
            // Fix: use full trimmed string for segmento fallback (not just first word)
            segmento: existingSegment && existingSegment.toString().trim() ? existingSegment.toString().trim() : "N/D"
        };
    }

    if (rawText.includes("/")) {
        const pieces = rawText.split("/");
        // Fix: use full string before "/" (not just first word) so "Otras fuentes/Otras fuentes"
        // correctly yields canal = "Otras fuentes" instead of just "Otras"
        const canal = pieces[0].trim() || "Otros";

        const afterSlash = pieces.slice(1).join("/").trim();
        const segmento = afterSlash || "N/D";

        return { canal, segmento };
    } else {
        const canal = rawText || "Otros";
        const segmento = existingSegment && existingSegment.toString().trim() ? existingSegment.toString().trim() : "N/D";
        return { canal, segmento };
    }
}
...
// TODO: confirmar con Cibertec si aplica — pesos: Disponibilidad 40%, Críticas 30%, Tiempo Caído 20%, Frecuencia 10%
function calculateGlobalHealthScore(availability, criticalOutages, totalOutageDurationMin, avgGap) {
    // 1. Availability (40%): // TODO: confirmar con Cibertec si aplica
    let uptimeScore = 40 * (availability / 100);

    // 2. Critical Outages (30%): Deduct 10 points per critical outage // TODO: confirmar con Cibertec si aplica
    let criticalScore = Math.max(0, 30 - (criticalOutages * 10));

    // 3. Total Outage Time (20%): Deduct based on accumulated outage hours (24h = 1440m is 0 points) // TODO: confirmar con Cibertec si aplica
    let durationScore = Math.max(0, 20 * (1 - (totalOutageDurationMin / 1440)));

    // 4. Frequency between leads (10%): Deduct if average gap exceeds 5 minutes (deduct 0.5 pt per min) // TODO: confirmar con Cibertec si aplica
    let avgGapScore = 10;
    if (avgGap > 5) {
...
function processRecords() {
    ...
    // Después: piso mínimo en el denominador (evita explosión cuando activeTimeMin ≈ 0)
    let rateLeadsPerMin = activeTimeMin > 1
        ? (recordsToProcess.length / activeTimeMin)
        : (recordsToProcess.length / totalPeriodTimeMin);
    ...
    for (let i = 1; i < recordsToProcess.length; i++) {
        ...
        if (diffMin >= 0) {
            ...
            if (diffMin > 5) {
                ...
                // Después: además de isNaN, techo de sanidad (nunca más leads afectados que el total del archivo)
                let leadsAfectadosEst = Math.round(rateLeadsPerMin * diffMin);
                if (isNaN(leadsAfectadosEst) || leadsAfectadosEst < 0) leadsAfectadosEst = 0;
                leadsAfectadosEst = Math.min(leadsAfectadosEst, recordsToProcess.length);
...
function updateMappingBadges() {
    ...
    Object.keys(targetColumns).forEach(prop => {
        ...
        let isRequired = requiredColumns.includes(friendlyName);

        let badgeClass, icon, matchedText;

        if (isMapped) {
            badgeClass = "bg-green-50 border-green-200 text-green-700";
            icon = "✅";
            matchedText = `<span class="text-[10px] text-gray-500 font-normal block truncate" title="${fileMetadata.mappedCols[friendlyName]}">Original: ${fileMetadata.mappedCols[friendlyName]}</span>`;
        } else if (isRequired) {
            badgeClass = "bg-red-50 border-red-200 text-red-700";
            icon = "❌";
            matchedText = '<span class="text-[10px] text-red-400 font-normal block">Falta en archivo (requerido)</span>';
        } else {
            badgeClass = "bg-gray-50 border-gray-200 text-gray-500";
            icon = "➖";
            matchedText = '<span class="text-[10px] text-gray-400 font-normal block">No aplica para este CRM (opcional)</span>';
        }
...
function updateKPIDOM(totalLeads, totalOutages, maxGap, maxGapStart, maxGapEnd, avgGap, continuity, activeIncidents = null, activeRecords = null) {
    ...
    continuityCard.className = "cib-card bg-gradient-to-br from-white to-gray-50/10 border-l-4 flex flex-col";
    if (continuity >= 98) {
        ...
        continuityCard.classList.add("border-l-cib-blueDark");
        continuityEl.className = "text-2xl font-black text-cib-blueDark mt-1 block";
    }
    ...
    bannerEl.className = "cib-card p-4 flex flex-col md:flex-row justify-between items-center gap-4 transition-all duration-300";
    ...
    if (healthScore >= 90) {
        ...
        bannerEl.classList.add("border-l-4", "border-l-cib-blueDark", "bg-red-50/10");
    }
    ...
    for (let r = 0; r < 7; r++) {
        ...
        for (let c = 0; c < 24; c++) {
            ...
            if (count > 0) {
                ...
                if (intensity <= 0.25) {
                    bgColor = "style='background-color: #E6F4FA;'"; // light Cibertec blue
                    textColor = "text-sky-800";
                } else if (intensity <= 0.5) {
                    bgColor = "style='background-color: #B3E5FC;'"; // mid-light Cibertec blue
                    textColor = "text-sky-950 font-semibold";
                } else if (intensity <= 0.75) {
                    bgColor = "style='background-color: #0075B0;'"; // Cibertec brand medium blue
                    textColor = "text-white font-bold";
                } else {
                    bgColor = "style='background-color: #004165;'"; // Cibertec brand dark blue
        ...
        data: {
            ...
            datasets: [
                ...
                },
                {
                    ...
                    data: incsTimeline,
                    borderColor: "#004165",
                    backgroundColor: "rgba(0, 65, 101, 0.05)",
                    borderWidth: 2,
                    yAxisID: "y1",
                    tension: 0.15,
                    fill: false
    ...
    for (let i = startIdx; i < endIdx; i++) {
        ...
        tbody.innerHTML += `
            <tr class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-3 py-3 font-semibold text-cib-dark">${item.duration.toFixed(1)} min</td>
...
function exportReportCSV() {
    ...
    link.setAttribute("download", `Incidentes_CRM_Cibertec_${new Date().toISOString().slice(0, 10)}.csv`);
...
function exportReportXLSX() {
    ...
    XLSX.writeFile(workbook, `Incidentes_CRM_Cibertec_${new Date().toISOString().slice(0, 10)}.xlsx`);
}
...
function exportReportPDF() {
    ...
    const opt = {
        ...
        filename: `Reporte_Intermitencias_CRM_Cibertec_${new Date().toISOString().slice(0, 10)}.pdf`,
...
function updateDiagnosticsTable(filteredInc = null, filteredRec = null, continuityVal = null) {
    ...
    // Después: usa el mismo universo (filas crudas totales) y clampa a 0-100%
    let totalRawRows = rawRecords.length;
    let dataErrors = (qualityAudit.invalids || 0) + (qualityAudit.duplicates || 0) + (qualityAudit.missings || 0);
    let dataQuality = totalRawRows > 0 ? Math.max(0, 1 - (dataErrors / totalRawRows)) : 1;
