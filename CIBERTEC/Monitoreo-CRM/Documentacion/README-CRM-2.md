...
El proyecto **Monitor de Caídas e Intermitencias CRM - Cibertec** está diseñado para analizar la consistencia y continuidad del flujo de leads recibidos en el CRM mediante la carga de un archivo (CSV o Excel). A continuación, se detallan cuáles son las columnas utilizadas y la lógica operativa del sistema.
...
*   **Para el cálculo de marcas de tiempo y detección de caídas (Críticos):**
    *   **Fecha** (`Fecha` / `fecha_creacion` / `fecha_registro` / etc.): Columna clave para obtener el día en el que entró el lead.
    *   **Hora** (`Hora` / `hora`): Columna clave con la hora exacta (por ejemplo, `14:35:10` o `02:35 PM`).
    *   *Nota: Cibertec no trae una columna de hora separada; el motor de parseo extrae la hora directamente de la cadena de Fecha si es la única columna temporal proporcionada.*

*   **Para auditoría de calidad de datos y unicidad:**
    *   **Código de Persona** (`Código de Persona` / `cod_persona` / etc.): Identificador del cliente. Se usa junto a la marca de tiempo para auditar duplicados exactos y detectar registros nulos o sin información crítica.
    *   **ID Conversación** (`ID Conversación` / `genesys_conversation_id` / etc.): ID de conversación alternativo de Genesys usado para validar transacciones.
...
Toda la lógica de procesamiento está detallada en [logic_and_formulas.txt](file:///c:/Users/yry/Downloads/Antigravity/Marketing%20Hub/CIBERTEC/Monitoreo-CRM/Documentacion/logic_and_formulas.txt). Los cálculos principales que realiza el archivo [main.js](file:///c:/Users/yry/Downloads/Antigravity/Marketing%20Hub/CIBERTEC/Monitoreo-CRM/js/main.js) son:
